...
export function toShort(value: number): number {
  return (value << 16) >> 16;
}
...
export function intRemainder(dividend: number, divisor: number): number {
  if (divisor === 0) throw new RangeError('/ by zero');
  return (dividend - intDiv(dividend, divisor) * divisor) | 0;
}
...
// Compatibility aliases for already ported code and the original tests.
export const int32 = toInt;
export const int16 = toShort;
export const int8 = toByte;
export const idiv = intDiv;
export const irem = intRemainder;

export interface RecordStoreBackend {
...
const memoryBackend: RecordStoreBackend = {
  getItem: (key) => memory.get(key) ?? null,
  setItem: (key, value) => { memory.set(key, value); },
  removeItem: (key) => { memory.delete(key); },
};

function defaultBackend(): RecordStoreBackend {
...
function decode(value: string): Uint8Array {
  if (value.length % 2 !== 0 || !/^[0-9a-f]*$/i.test(value)) throw new Error('Corrupt RecordStore data');
...
export class RecordStore {
  private static backend: RecordStoreBackend = defaultBackend();
  ...
  static openRecordStore(name: string, createIfNecessary: boolean): RecordStore {
    ...
    if (stored === null) {
      if (!createIfNecessary) throw new Error(`RecordStore does not exist: ${name}`);
    ...
    if (!Array.isArray(parsed) || !parsed.every((record) => typeof record === 'string')) {
      throw new Error(`Corrupt RecordStore: ${name}`);
    }
    return new RecordStore(name, parsed);
  }

  static useBackend(backend: RecordStoreBackend): void {
  ...
  private persist(): void {
    RecordStore.backend.setItem(this.key, JSON.stringify(this.records));
  }
